...
function ReviewsManagement() {
  ...
  const handleDelete = (review) => {
    if (window.confirm(t('admin.remove') + ' ' + t('admin.reviews') + '?')) {
      AxiosClient.delete(`/admin/reviews/${review.id}`)
        .then(() => {
          setMessage(t('admin.reviews') + ' ' + t('admin.removed') + ' ' + t('common.success'));
          fetchReviews();
        })
        .catch((error) => {
  ...
  return (
    <div className="px-5 mx-auto max-w-[1366px]">
      <h1 className="text-3xl font-bold text-[#444] mb-8">{t('admin.reviews')}</h1>
      <AdminTable columns={columns} data={reviews} actions={actions} loading={loading} />
    </div>
  );
}
